...
TestStvorec::TestStvorec(const nlohmann::json &parameters) :
  Agent()
{
  ...
  m_interface.position.x = (float)(0);
  m_interface.position.y = (float)(0);
  ...
  LOG(INFO) << "test stvorec initialized";
}

/** \brief Napíše do logu, že  bola zavolaná táto funkcia a čaká 5 ms. */
void TestStvorec::process(ae::Environment &env)
{
  (void)env;
...
uint16_t TestStvorec::assigned_type() const
{
  // plugin.cpp chcecks that this config entry exists
  return ae::config::get["agent_list"]["teststvorec"]["interface_type"];
}
